...
const blockCheck = async (solscanEndpoint, timeThreshold) => {
  ...
  try {
    ...
    if (!data || !data[0]) {
      return {
        status: ERROR,
        error: `${PREFIX} Latest Block API is down. Response data ${JSON.stringify(
          data
        )}`,
      };
    }
  ...
  if (latestBlockNumber) {
    ...
    if (!data || !data.currentSlot || !data.result) {
      return {
        status: ERROR,
        error: `${PREFIX} BlockDetail API is down. Response data ${JSON.stringify(
          data
        )}`,
      };
    }
...
const transactionCheck = async (solscanEndpoint, timeThreshold) => {
  try {
    ...
    if (!txDetail || txDetail.txHash != latestTx.transaction.signatures[0]) {
      return {
        status: ERROR,
        error: `${PREFIX} TransactionDetail API is down. Response data ${JSON.stringify(
          txDetail
        )}`,
      };
    }
  ...
  try {
    ...
    if (!data || !data[0] || !data[0].txHash) {
      return {
        status: ERROR,
        error: `${PREFIX} Failed to get transactions of account ${SAMPLE_ADDRESS}. Response data ${JSON.stringify(
          data
        )}`,
      };
    }
...
const splTransferCheck = async (solscanEndpoint, timeThreshold) => {
  try {
    ...
    if (!data || !data.data || !data.data[0] || !data.data[0].tokenAddress) {
      return {
        status: ERROR,
        error: `${PREFIX} Failed to get SplTransfer of account ${SAMPLE_ADDRESS}. Response data ${JSON.stringify(
          data
        )}`,
      };
    }
...
const solTransferCheck = async (solscanEndpoint, timeThreshold) => {
  try {
    ...
    if (!data || !data.data || !data.data[0] || !data.data[0].txHash) {
      return {
        status: ERROR,
        error: `${PREFIX} Failed to get SolTransfer of account ${SAMPLE_ADDRESS}. Response data ${JSON.stringify(
          data
        )}`,
      };
    }
...
const accountCheck = async (solscanEndpoint, timeThreshold) => {
  try {
    ...
    if (!data || !data.type || data.account != SAMPLE_ADDRESS) {
      return {
        status: ERROR,
        error: `${PREFIX} Failed to get AccountData of account ${SAMPLE_ADDRESS}. Response data ${JSON.stringify(
          data
        )}`,
      };
    }
...
const tokenCheck = async (solscanEndpoint, timeThreshold) => {
  ...
  try {
    ...
    if (!data || !data.data || !data.data[0] || !data.data[0].mintAddress) {
      return {
        status: ERROR,
        error: `${PREFIX} Failed to get TokenList. Response data ${JSON.stringify(
          data
        )}`,
      };
    }
  ...
  try {
    ...
    if (!data || !data.symbol) {
      return {
        status: ERROR,
        error: `${PREFIX} Failed to get TokenMetadata of ${SAMPLE_TOKEN}. Response data ${JSON.stringify(
          data
        )}`,
      };
    }
